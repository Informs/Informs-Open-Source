...
### render the index page for the portfolio
    ...
    use InhaleRead qw( getFolioDetails getFolioUnits getUnit getTags );
...
### perform actions...    
    ...
    { 
        $template1{'portfolioLogo'} = qq(<img src="$folioInfo{portfolioCustomLogo}" style="border:none;" alt="[ portfolio logo ]" />); 
    }      
    ...
    { 
        $image = '<p /><img src="'.$folioInfo{accountImage}.'" style="border:none;" />'; 
    }
...
### DISPLAY UNITS...
    ...
    foreach my $line (@units) 
    {
        ...
        $template2{unitLink} = 'jump.pl?'.$folio.'-'.$unit->{unitNumber};
        ...
        if($userIsAdmin) {
            ...
            $admin .= '&nbsp;<b>link</b>: <a title="link to this unit"  href="jump.pl?'.$folio.'-'.$objectID.'" target="_blank">'.$user->{pathToCGI}.'jump.pl?'.$folio.'-'.$objectID.'  </a>';
            $admin .= qq(<br />&nbsp;<b>options:</b> <b><a href="editunit.pl?unit=$objectID&amp;folio=$folio" title="edit this unit">edit</a></b>);           
            $admin .= qq( | <b><a href="copyunit.pl?from=$folio&amp;unit=$objectID&amp;folio=$folio" title="make a copy of this unit">copy</a></b>);
      ...
      my $myTags="";    
       $myTags =~ s/,/ /g;
        $myTags=getTags(unit =>$objectID);
         if ($myTags =~/[a-zA-Z]/) {
     
          $admin .=qq( <br /><br /><b>Current tags: </b> $myTags);
}
          $admin .=qq (<form action="$user->{pathToCGI}insertags.pl" method="post" >
                                  Add tags (comma separated)
                                  <input type="text" name="tags" value="" size="60" maxlength="255" />
                                    <input type="hidden" name="title" value="$title" size="" maxlength="" />
                                    <input type="hidden" name="unit" value="$objectID" >
                                    <input type="hidden" name="folio" value="$folio" >
                                   <input type="submit" class="tagsubmit" name="thebutton" value=" add tags" name="submit" />
                                 </form>);
            $admin .= '</td</tr>';
    ...
    if( $folioInfo{portfolioParent} ) 
    {
        ...
        $template1{subAccounts} .= qq(<ul>);
        $template1{subAccounts} .= '</ul>';
    }
 
    $template1{subAccounts}.=qq(  <a href="$user->{pathToCGI}portfolio2.pl?folio=$folio">Order by id</a> | <b id="clicky" style="font-weight:normal; color: #007F71;cursor:pointer; white-space: nowrap;" onclick="hider('portfoliotext');">Hide description</b> |);
